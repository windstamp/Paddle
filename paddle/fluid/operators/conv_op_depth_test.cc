...
template <typename T>
void TestDepthwiseConv2D(const platform::DeviceContext& ctx, const bool use_cudnn = false) {
  ...
  desc_fwd.SetAttr("strides", std::vector<int>({conv_stride, conv_stride}));
  desc_fwd.SetAttr("paddings", std::vector<int>({conv_padding, conv_padding}));
  desc_fwd.SetAttr("dilations", std::vector<int>({conv_dilation, conv_dilation}));
  ...
  desc_bwd.SetAttr("strides", std::vector<int>({conv_stride, conv_stride}));
  desc_bwd.SetAttr("paddings", std::vector<int>({conv_padding, conv_padding}));
  desc_bwd.SetAttr("dilations", std::vector<int>({conv_dilation, conv_dilation}));
...
TEST(test_conv2d_op, cpu_place) {
  ...
  TestDepthwiseConv2D<float>(ctx, false);
}
