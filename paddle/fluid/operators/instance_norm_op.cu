  ...
  void Compute(const framework::ExecutionContext &ctx) const override {
    ...
    PADDLE_ENFORCE_CUDA_SUCCESS(
        platform::dynload::miopenBatchNormalizationForwardTraining(
            ...
            in_param_desc_, 
            const_cast<void *>(static_cast<const void*>(scale_tmp.template data<BatchNormParamType<T>>())),
            const_cast<void *>(static_cast<const void*>(bias_tmp.template data<BatchNormParamType<T>>())), 
            0, 
            nullptr,
            nullptr, 
            epsilon,
